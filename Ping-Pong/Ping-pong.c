...
int main() {
  ...
  while (1) {
    ...
    if (scanf("%c", &command) == 1 || command != '\n') {
      ...
      if ((command == 'A') || (command == 'Z') || (command == 'K') ||
          (command == 'M') || (command == ' ')) {
        ...
        paddleCal(command, &paddle1_y,
                  &paddle2_y);  // Изменяет положение ракетки
      }
    }
...
void ballCalc(int* x, int* y, int* move, int paddle1_y, int paddle2_y,
  ...
  switch (*move) {  // Обработка отражения
    case 0:
      if (*x == 80 ||
          (*x == PADDLE2_X && *y >= paddle2_y - 1 && *y <= paddle2_y + 1)) {
    ...
    case 3:
      if (*x == 1 ||
          (*x == PADDLE1_X && *y >= paddle1_y - 1 && *y <= paddle1_y + 1)) {
        *move = 0;
      }
      if (*y == 1) {
        *move = 2;
...
void graph(int* ballX, int* ballY, int* paddle1_y, int* paddle2_y, int scoreF,
